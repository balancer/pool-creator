...
export const useReadPool = (pool: Address | null, chainId: number) => {
  ...
  // use data from api to populate "tokenInfo"
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [name, symbol, isRegistered, poolTokenInfo, poolConfig, normalizedWeights] = await Promise.all([
      ...
      for (let i = 0; i < poolTokenInfo[0].length; i++) {
        ...
        let tokenInfo = tokenList?.find(token => token.address.toLowerCase() === address.toLowerCase());
        ...
        if (!tokenInfo) {
          ...
          tokenInfo = {
            ...
            isBufferAllowed: false, // should be fine for init from previously deployed pool since only supporting init w/ actual tokens
