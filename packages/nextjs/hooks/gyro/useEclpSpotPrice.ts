...
export const useEclpSpotPrice = () => {
  const { updateEclpParam, eclpParams, tokenConfigs } = usePoolCreationStore();
  const { usdPerTokenInput0, usdPerTokenInput1 } = eclpParams;

  // fetch usd per token from API
  const { tokenUsdValue: usdPerToken0 } = useTokenUsdValue(tokenConfigs[0].address, "1");
  const { tokenUsdValue: usdPerToken1 } = useTokenUsdValue(tokenConfigs[1].address, "1");

  const { data: currentRateToken0 } = useFetchTokenRate(tokenConfigs[0].rateProvider);
  const { data: currentRateToken1 } = useFetchTokenRate(tokenConfigs[1].rateProvider);
  ...
  const shouldUseUnderlyingToken0 = currentRateToken0 && usdPerToken0 && !tokenConfigs[0].useBoostedVariant;
  const shouldUseUnderlyingToken1 = currentRateToken1 && usdPerToken1 && !tokenConfigs[1].useBoostedVariant;
