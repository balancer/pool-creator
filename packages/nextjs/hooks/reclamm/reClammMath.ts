...
export function calculateLowerMargin(params: {
  ...
  const c =
    marginPercentage *
    (Math.pow(params.virtualBalanceA, 2) - (params.invariant * params.virtualBalanceA) / params.virtualBalanceB);
...
export function calculateUpperMargin(params: {
  ...
  const c =
    (Math.pow(params.virtualBalanceA, 2) - (params.virtualBalanceA * params.invariant) / params.virtualBalanceB) /
    marginPercentage;
...
export function calculateOutGivenIn(params: {
  ...
  const balances = [params.balanceA + params.virtualBalanceA, params.balanceB + params.virtualBalanceB];
...
export const recalculateVirtualBalances = (params: {
  ...
  const fixedSecondsSinceLastInteraction =
    ...
    (params.simulationParams.secondsSinceLastInteraction % params.simulationParams.simulationSecondsPerBlock);
  ...
  const isPriceRatioUpdating =
    ...
    (params.simulationParams.simulationSeconds <= params.updateQ0Params.endTime ||
      params.simulationParams.simulationSeconds - fixedSecondsSinceLastInteraction <= params.updateQ0Params.endTime);
  ...
  if (isPriceRatioUpdating) {
    ...
    if (isPoolAboveCenter) {
      ...
      newVirtualBalanceB = (params.balanceB * newVirtualBalanceA) / (params.balanceA * poolCenteredness);
    } else {
      const a = Math.sqrt(newPriceRatio) - 1;
      ...
      newVirtualBalanceA = (params.balanceA * newVirtualBalanceB) / (params.balanceB * poolCenteredness);
    }
  }
...
/**
 * Matt derrived this function from reclamm math simulator repo
 */
export function calculateInitialBalances({
  ...
  // 2. Calculate ideal virtual balance A
  const idealVirtualBalanceA = defaultMaxBalanceA / (Math.sqrt(priceRatio) - 1);

  // 3. Calculate ideal virtual balance B
  const idealVirtualBalanceB = minPrice * (defaultMaxBalanceA + idealVirtualBalanceA);

  // 4. Calculate ideal balance B
  const idealBalanceB =
    Math.sqrt(targetPrice * (defaultMaxBalanceA + idealVirtualBalanceA) * idealVirtualBalanceB) - idealVirtualBalanceB;

  // 5. Calculate ideal balance A
  const idealBalanceA = (idealBalanceB + idealVirtualBalanceB - idealVirtualBalanceA * targetPrice) / targetPrice;
