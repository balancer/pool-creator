...
function isSmallPercentage(value: Numberish, { isPercentage = false }: { isPercentage?: boolean } = {}): boolean {
...
export function safeTokenFormat(amount: bigint | null | undefined, decimals: number, opts?: FormatOpts): string {
  if (!amount) return "-";
...
export const isValidNumber = (value: string): boolean => isNumber(toNumber(value)) && !isNaN(toNumber(value));
